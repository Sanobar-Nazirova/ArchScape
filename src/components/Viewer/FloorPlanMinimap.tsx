...
interface FloorPlanMinimapProps {
  ...
  isEditMode?: boolean; // true in the editor, false in presentation
}

export default function FloorPlanMinimap({
  floorPlan, currentSceneId, currentYaw, onSceneChange, isEditMode = false,
  ...
  const handleMapClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!isEditMode || !tagging || !currentSceneId || !imgRef.current) return;
    const rect = imgRef.current.getBoundingClientRect();
    const x = (e.clientX - rect.left)  / rect.width;
    const y = (e.clientY - rect.top)   / rect.height;
    setFloorPlanMarker(currentSceneId, Math.max(0, Math.min(1, x)), Math.max(0, Math.min(1, y)));
    setTagging(false);
  };
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    cursor: isCurrent && isEditMode && !tagging ? 'default' : 'pointer',
                    zIndex: isCurrent ? 2 : 1,
                  }}
                  onClick={e => {
                    e.stopPropagation();
                    if (!isCurrent) onSceneChange(marker.sceneId);
                  }}
                  title={scene?.name}
                >
                  ...
                  <button
                    ...
                    className={[
                      ...
                      tagging
                        ...
                        : 'text-nm-accent hover:bg-nm-accent/10',
                    ].join(' ')}
                  >
                    Move
                  </button>
                  <button
                    onClick={() => { removeFloorPlanMarker(currentSceneId); setTagging(false); }}
                ...
                <>
                  ...
                  <button
                    ...
                    className={[
                      ...
                      tagging
                        ? 'bg-nm-accent text-white'
                        : 'bg-nm-accent/20 text-nm-accent hover:bg-nm-accent/30',
                    ].join(' ')}
                  >
                    {tagging ? 'Cancel' : 'Tag location'}
                  </button>
                </>
              )}
            </div>
